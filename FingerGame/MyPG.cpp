...
namespace MyPG
{
	...
	//以降ゲーム用エフェクト
		case 10://【ゆびすま～】表示
			if (auto e = Effect00::Object::Create(true)) {
				e->filename = "data/image/FingerSmash282x106.png";
				...
				//角度
				e->angle = ML::ToRadian(-5.0f);
				e->addAngle = ML::ToRadian(1.0f / 9.0f);
...
*/
	...
	MyGameEngine::MyGameEngine()
		:GameEngine(
			...
			480,							//	2D描画用解像度X(現在非対応）
			270,							//	2D描画用解像度Y(現在非対応）
			"2022年度_進級制作_ゆびすまげえむ_長谷川勇一朗",	//	ウインドウタイトル
			"GPG",							//	ウィンドウクラス名
	...
	bool MyGameEngine::Initialize(HWND wnd_)
	{
		...
		{
			bgm::Init();
			bgm::LoadFile("bgm", "./data/bgm/bgm.mp3");
			bgm::Play("bgm");
			bgm::VolumeControl("bgm", 80);

			se::Init(wnd_);
			se::LoadFile("Select", "./data/se/Select.wav");
			se::LoadFile("decision", "./data/se/decision.wav");
			se::SetVolume("Select", 80);
			se::SetVolume("decision", 50);

			//以下ボイス
			//----
			// xxxx = player or cpu
			// xxxxは「ゆびすま」の読み上げ/90フレームかける
			// xxxx0-4はゆびすま〇の数字の読み上げ
			//----
			//プレイヤー側
			se::LoadFile("player", "./data/voice/player.wav");
			se::LoadFile("player0", "./data/voice/player_0.wav");
			se::LoadFile("player1", "./data/voice/player_1.wav");
			se::LoadFile("player2", "./data/voice/player_2.wav");
			se::LoadFile("player3", "./data/voice/player_3.wav");
			se::LoadFile("player4", "./data/voice/player_4.wav");

			//
			se::LoadFile("cpu", "./data/voice/cpu.wav");
			se::LoadFile("cpu0", "./data/voice/cpu_0.wav");
			se::LoadFile("cpu1", "./data/voice/cpu_1.wav");
			se::LoadFile("cpu2", "./data/voice/cpu_2.wav");
			se::LoadFile("cpu3", "./data/voice/cpu_3.wav");
			se::LoadFile("cpu4", "./data/voice/cpu_4.wav");
		...
		//デバッグ用フォントの準備
		this->debugFont = DG::Font::Create("ＭＳ ゴシック", 12, 32);
